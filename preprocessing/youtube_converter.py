...
TRANSCRIPTION_RESULTS = namedtuple('transcription_results', ['start_time',
                                                             'end_time',
                                                             'transcription',
                                                             'confidence'])
AUDIO_METADATA = namedtuple('audio_metadata', ['channels',
                                               'sample_width',
                                               'frame_rate'])
...
class RssWebDriverAudioExtractor(WebDriverMultiMediaExtractor):
    ...
    def extract_multimedia(self, url):
        self.driver.get(url)
        download_link = self.driver.find_element_by_class_name('download-link').get_attribute('href')
...
class SimpleWebDriverAudioExtractor(WebDriverMultiMediaExtractor):
    ...
    def extract_metadata(self, url):
        raise NotImplementedErrorstr_converter = ReadwiseConverter()
    def __init__(self):
        super().__init__()

    def extract_multimedia(self, url):
        self.driver.get(url)
        self.driver.switch_to.frame(self.driver.find_element_by_xpath('//iframestarts-with(@src, "https://html5-player.libsyn.com/embed")'))
...
class YoutubeDLMultiMediaExtractor(object):
    ...
    def extract_audio_metadata(self, fileloc):
        ...
        return AUDIO_METADATA(channels=audio_obj.channels, sample_width=audio_obj.sample_width, frame_rate=audio_obj.frame_rate)
    ...
    def extract(self, url):
        ...
        downloaded_location = '/tmp/seamless_downloads/{0}-{1}.flac'.format(info['id'], info['extractor'])
...
class SpeechToTextWrapper(object):
    ...
    def transcribe_file(self, speech_file, audio_metadata):
        ...
        for result in response.results:
            ...
            transcription.append({'start_time': min_start,
                                  'end_time': max_end,
...
def generate_youtube_transcript_docs(url: str, downloader_config: dict = None):
    ...
    url_texts = speech_to_text.transcribe_file(speech_file=url_gs_loc, audio_metadata=url_audio_meta)

    # massage data to proper format
    youtube_docs = []
    for url_text in url_texts:
        url_dict = str_converter.convert(url.transcription, meta = {'src_path': hash_string, 'title': url_audio_info.title,
                                         'author': url_audio_info.author, 'start_time': url_text.start_time, 'end_time':
                                         url_text.end_time, 'confidence': url_text.confidence})
