...
def test_generate_docx_docs(test_file_list):
    expected_docx_docs = ['A DeFi crash course for normies_ Crypto markets since 2017.docx']
    gen_docs = generate_docx_docs(test_file_list)
    assert all([True if x['meta']['file_name'] in expected_docx_docs else False for x in gen_docs])
...
def test_generate_md_docs(test_file_list):
    ...
    assert all([True if x['meta']['file_name'] in expected_md_docs else False for x in gen_docs])
...
def test_generate_pdf_docs(test_file_list):
    ...
    assert all([True if x['meta']['file_name'] in expected_pdf_docs else False for x in gen_docs])
...
def test_generate_txt_docs(test_file_list):
    ...
    assert all([True if x['meta']['file_name'] in expected_txt_docs else False for x in gen_docs])
...
def test_preprocess_test(test_file_list):
    ...
    processed_docs_filenames = list(set([x['meta']['file_name'] for x in processed_docs]))
    expected_filenames = ['A DeFi crash course for normies_ Crypto markets since 2017.docx', 'activation function.md', 'CoAtNet: Marrying Convolution and Attention.pdf', 'sample3.txt']
